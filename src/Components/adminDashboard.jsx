...
import { FiUser, FiLogOut, FiDollarSign, FiAlertCircle, FiMapPin } from "react-icons/fi";
import "../style.css";
...
function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        ...
        const [statsRes, transactionsRes, lowBalanceRes] = await Promise.all([
          adminAPI.getStats(),
  ...
  const handleGenerateReport = async (type) => {
    try {
      ...
      let response;
      if (type === 'financial') {
        response = await reportAPI.getFinancialReport(reportType);
      } else if (type === 'collection') {
        response = await reportAPI.getCollectionReport(collectionDays);
      }
    ...
    <div className="admin-dashboard-container">
      ...
      <div className="management-section">
        ...
        <div className="admin-tabs">
          <button 
            className={`admin-tab-btn ${activeTab === "passengers" ? "active" : ""}`}
            onClick={() => setActiveTab("passengers")}
          >
            Passengers
          </button>
          <button 
            className={`admin-tab-btn ${activeTab === "balance" ? "active" : ""}`}
      ...
      {loading ? (
        <p style={{color: 'white', textAlign: 'center', fontSize: '18px'}}>Loading admin data...</p>
      ) : (
        <>
          {/* PASSENGERS TAB */}
              ...
              <div className="stats-cards">
                ...
                <div className="admin-stat-card">
                  ...
                  <div className="stat-content">
                    ...
                    <p className="stat-value">{(stats.totalCollection / 100).toLocaleString()}</p>
                  </div>
                </div>
                ...
                <div className="admin-stat-card">
                  ...
                  <div className="stat-content">
                    <p className="stat-label">Low Balance Alert</p>
                      ...
                      <div key={t.id} className="transaction-card">
                        <div className="transaction-details">
                          ...
                          <div className="transaction-info">
                            ...
                            <p className="transaction-passenger">{t.passenger} RFID {t.email}</p>
                            <p className="transaction-datetime">
                              {new Date(t.timestamp).toLocaleDateString()} {new Date(t.timestamp).toLocaleTimeString()}
                            </p>
                          </div>
                        </div>
                        <p className="transaction-amount">Rs {(t.amount / 100).toFixed(0)}</p>
                      </div>
                    ))
                  )}
                </div>
                  ...
                  <>
                    {lowBalanceUsers.slice(0, 1).map((u) => (
                      <div key={u.id} className="low-balance-card">
                        ...
                        <p className="user-balance">Rs {(u.balance / 100).toFixed(0)}</p>
                      </div>
                    ))}
                    <button 
                      className="send-alert-btn"
                      onClick={() => handleSendAlert(lowBalanceUsers[0]?.id)}
                    >
            ...
            <div className="balance-tab-content">
              ...
              <div className="stats-cards" style={{marginTop: '20px'}}>
                <div className="admin-stat-card">
                  ...
                  <div className="stat-content">
                    <p className="stat-label">Total Collection</p>
                    <p className="stat-value">NPR {(stats.totalCollection / 100).toFixed(2)}</p>
                  </div>
                </div>
                <div className="admin-stat-card">
                  <span className="stat-icon">⚠️</span>
                  ...
                  <table className="report-table">
                    ...
                    <tfoot>
                      <tr>
                        <td>Total</td>
                        <td style={{textAlign: 'right'}}>
                          Rs {(reportData.data.reduce((sum, item) => sum + item.amount, 0) / 100).toFixed(2)}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
